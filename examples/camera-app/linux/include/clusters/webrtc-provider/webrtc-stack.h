...
public:
    ...
    virtual std::shared_ptr<WebRTCTrack> AddTrack(const std::string & mediaType)                       = 0;
};

// Abstract WebRTC stack interface
class WebRTCStack
{
public:
    virtual ~WebRTCStack() = default;

    virtual CHIP_ERROR Init()                                             = 0;
    virtual std::unique_ptr<WebRTCPeerConnection> CreatePeerConnection()  = 0;
    static std::unique_ptr<WebRTCStack> Create();
};