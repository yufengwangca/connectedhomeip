...
// Constants have been moved to the libdatachannel implementation
...
void WebRTCProviderManager::OnLocalDescription(const std::string & sdp, const std::string & type)
{
    if (mState == State::SendingAnswer && type != "answer")
    {
        return;
    }

    mLocalSdp = sdp;
    ChipLogProgress(Camera, "Local Description (%s):", type.c_str());
...
{
    mWebRTCStack = WebRTCStack::Create();
    mWebRTCStack->Init();

    mPeerConnection = mWebRTCStack->CreatePeerConnection();

    mPeerConnection->SetCallbacks(
        [this](const std::string & sdp, const std::string & type) { this->OnLocalDescription(sdp, type); },
...
CHIP_ERROR WebRTCProviderManager::HandleProvideOffer(const ProvideOfferRequestArgs & args, WebRTCSessionStruct & outSession)
{
    ...
    if (mWebrtcTransportMap.find(args.sessionId) == mWebrtcTransportMap.end())
    {
        mWebrtcTransportMap[args.sessionId] =
            std::unique_ptr<WebrtcTransport>(new WebrtcTransport(args.sessionId, mPeerId.GetNodeId()));
    }
    ...
    mPeerConnection->SetRemoteDescription(args.sdp, "offer");
...
CHIP_ERROR WebRTCProviderManager::HandleProvideAnswer(uint16_t sessionId, const std::string & sdpAnswer)
{
    ...
    mPeerConnection->SetRemoteDescription(sdpAnswer, "answer");
